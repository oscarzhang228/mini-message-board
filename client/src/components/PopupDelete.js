...
function PopupDelete(props) {
  ...
  const [messageDelete, setMessageDelete] = useState(-1);
  const selectMessage = (messageNum) => {
    setMessageDelete(messageNum - 1);
    if (messageNum === 1) {
      setClicked1(
    ...
    <div className="vh-100 vw-100 position-absolute d-flex align-items-center justify-content-center">
      ...
      >
        ...
        <div className="mt-5 d-flex">
          ...
          <button className="px-2 rounded buttons create" onClick={deleter}>
            Delete
          </button>
        </div>
      </div>
